...
try:
    ...
    from .models import WeeklyFlatResponse
    ...
    from fastapi_app.models import WeeklyFlatResponse  # type: ignore[no-redef]
    from fastapi_app.storage import default_store  # type: ignore[no-redef]

app = FastAPI(title="Weekly pt1h metrics API", version="0.1.0")
...
def pt1h_weekly(
    ...
    para = {k: _to_number(v) for k, v in rec.pt1hPara.model_dump().items()}
    return WeeklyFlatResponse(lotcount=rec.lotcount, wfCount=rec.wfCount, **para)
