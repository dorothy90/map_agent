...
Q-TIME 행은 FROM 공정과 TO 공정 그룹에 각각 한 번씩 포함한다. 두 이벤트는 동일한 `event_id`를 공유하고 역할을 `qtime_outgoing`과 `qtime_incoming`으로 구분한다. 이를 통해 LLM은 출발 공정 이후 지연과 도착 공정 진입 전 지연을 모두 볼 수 있으며, 단일 비교 분석에서 동일 사건을 두 건으로 세지 않는다.
